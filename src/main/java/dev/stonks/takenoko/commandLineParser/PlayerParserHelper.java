...
class PlayerParserHelper extends ArrayList<String> implements CommandLine.ITypeConverter<Player> {
    ...
    public Player convert(String s) {
        switch (s) {
            case "random" : return new RandomPlayer(count++);
            case "dumb" : return new DumbPlayer(count++);
            case "smart" : return new SmartPlayer(count++);
            case "smart3" : return new SmartPlayer(count++, 3);
            case "smart4" : return new SmartPlayer(count++, 4);
